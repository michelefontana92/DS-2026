...
class MaskLayer1d_Masked(nn.Module):
    """
    Masking for 1d inputs.
    ...
    Args:
      value: replacement value(s) for held out features.
             Can be:
               - scalar (float / 0-d tensor)
               - 1D tensor/array of shape (d,)
      append: whether to append the mask along feature dimension.
    """
    def __init__(self, value=0.0, append=False):
        super().__init__()
        self.append = append
    ...
    def forward(self, input_tuple):
        ...
        if v.ndim == 0:
            ...
            # (d,) -> make (1,d) to broadcast along batch
            v = v.unsqueeze(0)
        else:
            raise ValueError(f"MaskLayer1d: value must be scalar or 1D (d,), got shape {tuple(v.shape)}")
