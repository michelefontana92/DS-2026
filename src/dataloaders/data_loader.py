...
class DataModule(BaseDataLoader):
    ...
    def feature_names(self):
        return self.datasets.get('train').feature_names
    ...
    def get_cat_cols(self):
        return self.datasets['train'].cat_cols
    
    def get_cat_cols_mask(self):
        cat_cols = self.get_cat_cols()
        features_names = self.feature_names()
        mask = [1 if f in cat_cols else 0 for f in features_names]
        return mask
